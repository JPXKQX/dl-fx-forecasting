...
@click.argument('var', type=click.Choice(['mid', 'spread', 'increment']))
...
@click.option('--data_path', default="data/raw/", type=click.STRING, help="Path"
              ...
              metavar="<str>")
def main(base, quote, var, freqs, period, data_path):
    ...
    PlotCurrencyPair(base, quote, var, freqs, data_path).run(period)
...
def main_cdf(base, quote, which, period, data_path, rate, group_hourly):
    ...
    plotter = PlotCDFCurrencyPair(base, quote, which, rate, data_path)
    plotter.run(period, group_hourly)
